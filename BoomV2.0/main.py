...
class Window:
    ...
    def spawnBombs(self, bombAmount):
        ...
        sizeX, sizeY = Bomb.undetonated.get_size()
        bombSpawnzone = self.screen.get_rect(
        ).inflate(-(sizeX // 2), -(sizeY // 2))

        retries = 0
        while bombAmount > 0 and retries < 1000:
            ...
            bomb = Bomb(x, y)
            collisions = pygame.sprite.spritecollide(bomb, group, dokill=False)
            retries += 1
